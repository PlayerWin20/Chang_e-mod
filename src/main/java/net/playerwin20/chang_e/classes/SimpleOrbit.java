...
public class SimpleOrbit {
    ...
    public SimpleOrbit(@Nullable Cel parentBody, Vector3f localApoapsis, float eccentricity) {
    ...
    public float getThetaAfter(float startTheta, float deltaSeconds, float secondsPerOrbit) {
        float angularSpeed = (float)(2 * Math.PI / secondsPerOrbit);
        return startTheta + angularSpeed * deltaSeconds;
    }

    public Vector3f localPositionAfter(float startTheta, float deltaSeconds, float secondsPerOrbit) {
        return localPosition(getThetaAfter(startTheta, deltaSeconds, secondsPerOrbit));
    }
